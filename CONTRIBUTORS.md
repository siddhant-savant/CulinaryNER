# Contributors

## Core Team

### Siddhant Sawant

**Contributions:**
- Model architecture design and implementation
- spaCy training pipeline development
- Performance optimization and evaluation
- Training, evaluation, and inference scripts
- GitHub repository setup and maintenance

**Contact:**
- Email: siddhantsavant7@gmail.com
- LinkedIn: [linkedin.com/in/siddhant-sawant-343324180](https://linkedin.com/in/siddhant-sawant-343324180)
- GitHub: [@siddhant-savant](https://github.com/siddhant-savant)

---

### Akshatha Poojari

**Contributions:**
- Dataset collection and organization
- Data annotation supervision and quality control
- Annotation guidelines development
- Dataset statistics and analysis
- Usage examples and tutorials
- Technical documentation
- Documentation and README creation

**Contact:**
- Email: akshatha.poojari01@gmail.com
- LinkedIn: [linkedin.com/in/akshatha-poojari-32ab12198](https://linkedin.com/in/akshatha-poojari-32ab12198)
- GitHub: [@akshatha-25](https://github.com/Akshatha-25)

---

## How to Contribute

We welcome contributions! Areas for improvement:

- Additional cuisine types (French, Thai, Mexican, etc.)
- New entity categories (DIETARY_RESTRICTION, PRESENTATION)
- Multi-language support
- Error analysis and corrections
- Model improvements (BERT, transformers, etc.)

Please open an issue or pull request to discuss contributions.

---

## Citation

If you use CulinaryNER in your research or application, please cite:

```bibtex
@dataset{culinaryner2026,
  title={CulinaryNER: Named Entity Recognition for Culinary Text},
  author={Sawant, Siddhant and Akshatha, Poojari},
  year={2026},
  publisher={GitHub},
  url={https://github.com/siddhant-savant/CulinaryNER}
}
```
